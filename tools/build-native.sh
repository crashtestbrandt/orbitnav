...
platform_parts() {
	case "$1" in
		linux)   BUILT_NAME="liborbitnav.so";  SHIP_PREFIX="liborbitnav.linux";  SHIP_SUFFIX="x86_64.so" ;;
		windows) BUILT_NAME="orbitnav.dll";    SHIP_PREFIX="orbitnav.windows";   SHIP_SUFFIX="x86_64.dll"
		...
		macos)   BUILT_NAME="liborbitnav.dylib"; SHIP_PREFIX="liborbitnav.macos"; SHIP_SUFFIX="universal.dylib" ;;
...
build)
	...
	for p in "${profiles[@]}"; do
		...
		if [ "$PLATFORM" = macos ] && [ "$p" = profiling ]; then
			...
			( cd "$NATIVE" && cargo rustc "${CARGO_FLAG[@]}" -p orbitnav-godot \
			...
			( cd "$NATIVE" && cargo build "${CARGO_FLAG[@]}" -p orbitnav-godot --target x86_64-apple-darwin )
			( cd "$NATIVE" && cargo build "${CARGO_FLAG[@]}" -p orbitnav-godot --target aarch64-apple-darwin )
			...
			if [ "${#RUSTC_ARGS[@]}" -gt 0 ]; then
				( cd "$NATIVE" && cargo rustc "${CARGO_FLAG[@]}" "${TARGET_ARGS[@]}" -p orbitnav-godot -- "${RUSTC_ARGS[@]}" )
			else
				( cd "$NATIVE" && cargo build "${CARGO_FLAG[@]}" "${TARGET_ARGS[@]}" -p orbitnav-godot )
			fi
		...
		if [ "$PLATFORM" = windows ] && [ "$p" = profiling ]; then
			...
			[ -s "$pdb" ] || {
				...
				exit 1; }
			cp -p "$pdb" "$OUTDIR/orbitnav.pdb"
			printf 'build-native: shipping orbitnav.pdb beside the DLL\n'
		fi
