...
# platform, and nowhere else. `orbitnav.windows.x86_64.dll` was named by the descriptor and produced by no
...
DESCRIPTOR="addons/orbitnav_native/orbitnav.gdextension"
...
named="$(grep -oE 'res://addons/orbitnav_native/bin/[^"]+' "$DESCRIPTOR" \
	| sed 's|.*/||' | sort -u || true)"
if [ -z "$named" ]; then
