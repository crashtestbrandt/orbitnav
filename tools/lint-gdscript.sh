...
if [ ! -d "$PROJECT_DIR/addons/orbitnav" ]; then
	printf 'lint FAILED: %s/addons/orbitnav is missing -- run `just sync-addons` first.\n' "$PROJECT" >&2
	exit 1
fi

LOG_FILE="$(mktemp "${TMPDIR:-/tmp}/orbitnav-lint.XXXXXX")"
