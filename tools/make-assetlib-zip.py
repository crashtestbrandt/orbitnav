...
BIN = os.path.join("addons", "orbitnav_native", "bin")
...
# libraries beside it.
SKIP = {os.path.join("addons", "orbitnav_native", "binaries.json")}

os.makedirs(outdir, exist_ok=True)
out = os.path.join(outdir, "orbitnav-%s.zip" % version)

libs = 0
with zipfile.ZipFile(out, "w", zipfile.ZIP_DEFLATED) as z:
    for root in ("addons/orbitnav", "addons/orbitnav_native"):
