...
#
# The repo root is deliberately NOT a Godot project. OrbitNav is configured through an [orbitnav]
# block in project.godot, and a project that consumes the addon disagrees with the harness about those
# values on purpose. Two projects cannot share one settings block, so each is its own Godot project and
# gets its own COPY of the addon. `addons/orbitnav/` + `addons/orbitnav_native/` at the repo root are
# the single source of truth and the AssetLib payload; every copy under a project is a build artifact
# (gitignored).
#
...
# fatal, cryptic first-run failure ("Failed to load script ... res://addons/orbitnav/nav.gd"). A
# copy works everywhere with no configuration. Because the relative path `addons/<name>` is
# preserved, NOTHING needs path rewriting: the .gdextension entries, the `Nav` autoload path and
...
# what a game vendors. tools/test-harness is here for the same reason and by the same mechanism: one
# canonical copy of the tiny hand-rolled unit-test runner with drift checking beats a copy per project
# that quietly diverges.
PAYLOAD=(
	"addons/orbitnav:addons/orbitnav"
	"addons/orbitnav_native:addons/orbitnav_native"
